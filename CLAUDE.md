...
`-Aexeris.strict` (added 0.5.x, T11) opts into a completeness audit: a `javac` WARNING when an annotation attribute — or a whole annotation — is set but no generator consumes it. Well-defined because all SDK annotations are `@Retention(SOURCE)` (erased from bytecode → runtime/SPI/Core cannot read them → the build-time pipeline is the only possible consumer; an unconsumed attribute has zero effect). Two passes, answering different questions and using different wording so they stay separable:

1. **extracted-but-unconsumed** — backed by two conservative denylists in `ExerisDomainProcessor`: `INERT_ATTRIBUTES` (per-attribute) and `INERT_ANNOTATIONS` (whole-annotation, e.g. `@EventSourced` until its generator exists). When a generator starts consuming one of those, delete its registry entry in the same change (a stale entry produces a false "no effect" warning).
2. **never-read** (C0, 0.8.0) — driven by `EXTRACTED_ANNOTATIONS`, an **allowlist** of what the processor reads. Anything present on a visited element and absent from it is reported with no registry entry required; a new extraction must join the set in the same change that adds it. This is the half the call-site-driven design was structurally blind to: no extraction meant no call site, so 16 of the SDK's 35 annotations could not produce a warning by any path. `UNREAD_NOTES` only upgrades the generic reason to a specific one — never gates the warning. A `@Repeatable` container reports under its member's name, and reports nothing when that member is extracted.
