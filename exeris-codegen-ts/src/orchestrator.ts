...
/**
 * Emits the enum module (`types/enums.ts`): a const-object value, its literal-union
 * type, a `*DisplayNames` lookup, and a Zod `*Schema` — the real members the entity
 * types and Zod schemas reference. (The former stub shipped an empty `enum {}`.)
 */
export function generateEnumTypes(enums: EnumMetadataForGen[]): string {
  const lines: string[] = [
    '/**',
  ...
  // zod import at the top (after the file-header comment block).
...
export function buildGeneratedFiles(
  ...
  if (enums.length > 0) {
    appTree.push({ path: 'types/enums.ts', content: generateEnumTypes(enums) });
  }
