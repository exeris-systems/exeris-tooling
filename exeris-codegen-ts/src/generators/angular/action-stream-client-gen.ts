...
export class ActionStreamClientGenerator implements CodeGenerator {
  ...
  private renderActionStreamClients(
    ...
    lines.push(``);
    lines.push(`/**`);
    lines.push(` * A parsed SSE frame. \`event\` is the SSE event: name (the action's`);
    lines.push(` * streamEventType, or 'keep-alive' for the scaffold heartbeat); \`data\` is the`);
    lines.push(` * raw data: payload. Unlike native EventSource.onmessage (which drops NAMED`);
    lines.push(` * frames), this parser reads the event: line, so named events are delivered.`);
    lines.push(` */`);
    lines.push(`export interface StreamFrame {`);
