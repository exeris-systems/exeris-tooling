...
function generateAppComponent(domains: DomainMetadata[]): string {
  const navItems = domains.map((d) => {
    const plural = d.entityName.endsWith('s') ? d.entityName : d.entityName + 's';
    ...
    return { path: routePlural(d.entityName), label: plural, icon };
  });
...
// Kebab-cased route plural matching the nav-label guard at
// generateAppComponent: entity names already ending in 's' (e.g.
// `News`) do NOT get a second 's' appended, otherwise `News` would
// produce route `newss` (visible to users) while the sidebar label
// correctly stays `News`. Keep both sides in lockstep.
